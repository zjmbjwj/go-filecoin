...
func TestRetrievalDeal(t *testing.T) {
	...
	clientNode := makeNode(ctx, t, seed, chainClock)

	var sectorSealed bool
	for i := 0; i < 100; i++ {
	...
	require.True(t, sectorSealed)
	// Find out info of storage providers and make deal
	sps, err := clientNode.StorageProtocol.StorageClient.ListProviders(ctx)
	require.NoError(t, err)
	require.Len(t, sps, 1)
}